...
async function getFallbackPexelsImage(
  ...
  try {
    ...
    console.error('Error fetching Pexels image')
    return `pexels:${keyword}` // Return original source
  }
}
    ...
    if (file instanceof Blob) {
      ...
      if (file && typeof file === 'object' && 'base64Data' in file && 'mimeType' in file) {
        try {
          ...
          console.error('Error processing Vertex AI image data')
          throw vertexError
        }
      }
      // Try to handle common formats that might not be properly detected
        ...
        try {
          ...
          console.error('Error converting image buffer')
          blob = file as unknown as Blob
        }
        ...
        try {
          ...
          console.error('Error converting image data')
          blob = file as unknown as Blob
        }
      } else {
        blob = file as unknown as Blob
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (uploadResult.success && uploadResult.file) {
          ...
          console.error(`Upload failed after ${attempts} attempt(s)`)
          if (attempts >= maxAttempts) throw new Error(uploadResult.error || 'Failed to upload image')
        }
      } catch (uploadError) {
        console.error(`Upload error on attempt ${attempts}`)
