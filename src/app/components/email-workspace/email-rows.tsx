...
const EmailRows = () => {
  const rowOptions = [[12], [10, 2], [8, 4], [6, 6], [4, 8], [2, 10], [4, 4, 4], [3, 3, 3, 3], [3, 6, 3], [2, 2, 2, 2, 2, 2]]
  return (
...
const DraggableRow = ({ width }: DraggableRowProps) => {
  const [, drag] = useDrag(() => ({
    type: 'newRow',
    item: { type: 'newRow', id: uuidv4(), width: width },
  }))
  ...
  return (
    ...
    <button ref={drag} className="group flex w-full gap-2">
      {width.map((columnWidth, index) => (
        <div key={index} className="h-14 rounded-md bg-gray-200 transition-colors duration-200 group-hover:bg-gray-300 dark:bg-gray-700 dark:group-hover:bg-gray-600" style={{ flexBasis: `${(columnWidth / 12) * 100}%` }}>
