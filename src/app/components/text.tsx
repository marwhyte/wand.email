...
export function Text({ className, ...props }: React.ComponentPropsWithoutRef<'p'>) {
  return (
    <p
      data-slot="text"
      {...props}
      className={clsx('text-base/6 text-zinc-500 dark:text-zinc-400 sm:text-sm/6', className)}
    />
  )
}
...
export function TextLink({ className, ...props }: React.ComponentPropsWithoutRef<typeof Link>) {
  return (
    <Link
      {...props}
      className={clsx(className, 'text-blue-500 underline decoration-blue-500/50 data-[hover]:decoration-blue-500')}
    />
  )
}
