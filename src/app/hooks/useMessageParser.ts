...
export function useMessageParser(message: Message) {
  ...
  const setEmailState = (state: 'idle' | 'open' | 'closed' | 'done') =>
    setEmailStates((prev) => ({ ...prev, [message.id]: state }))

  useEffect(() => {
    const processMessage = async () => {
      const hasOpenTag =
        message.content.includes('<EMAIL') && message.content.match(/<EMAIL\s+[^>]*name=["'][^"']*["']/)
